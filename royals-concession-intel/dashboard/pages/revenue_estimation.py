...
from dashboard.components.theme import page_header, dark_card, section_header

# ---------------------------------------------------------------------------
# Cached data loaders (defined outside render so @st.cache_data works)
# ---------------------------------------------------------------------------
...
def render():
    page_header(
        "Revenue Estimation",
        "Revenue tracking, per-cap analysis, and benchmark gaps",
        "\U0001f4b0",
    )
    ...
    season = season_filter(inline=True)
    ...
    with col_left:
        ...
        fig_trend.update_traces(line_color="#7c3aed", line_width=3, mode="lines+markers")
    ...
    if not games.empty:
        section_header("Revenue Per Cap vs Attendance")
        ...
        # Try with trendline (requires statsmodels); fall back without
        try:
