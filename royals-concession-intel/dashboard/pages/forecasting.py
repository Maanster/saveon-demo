...
# ---------------------------------------------------------------------------
# Color palette (dark-theme Royals purple & gold)
# ---------------------------------------------------------------------------
LIGHT_PURPLE = "#1c2333"
ROYALS_PALETTE = [
    "#7c3aed", "#FFD700", "#6A0DAD", "#FFA500",
    "#9B30FF", "#FFB347", "#5C2D91", "#FFCC33",
]

# ---------------------------------------------------------------------------
# DB helpers (cached, outside render)
# ---------------------------------------------------------------------------
...
# ===========================================================================
def render():
    page_header("Game Day Forecasting", "ML-powered demand predictions and operational prep sheets", "\U0001f4c8")
    ...
    # --- Input widgets (inline instead of sidebar) ---
    section_header("Game Parameters")
    ...
    inp_row1_c1, inp_row1_c2, inp_row1_c3 = st.columns(3)
    with inp_row1_c1:
    ...
    inp_row2_c1, inp_row2_c2, inp_row2_c3 = st.columns(3)
    with inp_row2_c1:
    ...
    with prep_container:
        ...
        # --- Buffer callout ---
        buffer_border = "#FFD700" if buffer_pct >= 0.25 else "#28a745"
        st.markdown(
            f"""<div style="background: #1c2333;
                    border-left: 4px solid {buffer_border};
        ...
        section_header("Item-Level Prep Quantities")
        ...
        section_header("Stand Opening Plan")
        open_col, close_col = st.columns(2)
        with open_col:
            for s in stand_plan["open_stands"]:
                short = s.replace("SOFMC ", "")
                st.markdown(
                    f'<div style="background: #1c2333; padding: 0.5rem 0.8rem; border-radius: 4px; '
                    f'margin: 0.3rem 0; border-left: 4px solid #28a745; color: #e6edf3;">'
        ...
        with close_col:
            if stand_plan["close_stands"]:
                for s in stand_plan["close_stands"]:
                    ...
                    st.markdown(
                        f'<div style="background: #1c2333; padding: 0.5rem 0.8rem; border-radius: 4px; '
                        f'margin: 0.3rem 0; border-left: 4px solid #dc3545; color: #e6edf3;">'
        ...
        section_header("Stand-Level Stock Breakdown")
        if stand_breakdown:
        ...
        section_header("Comparable Historical Games")
    ...
    # ===========================================================================
    st.markdown('<div class="gold-divider"></div>', unsafe_allow_html=True)
    section_header("Demand Breakdown")
    ...
    with chart_col2:
        st.markdown(
            ...
            <div style="background: linear-gradient(135deg, #1c2333 0%, #161b22 100%);
                        border: 1px solid #7c3aed; border-left: 4px solid {GOLD};
                        padding: 1.5rem; border-radius: 8px; text-align: center; margin-top: 1rem;">
                <p style="color: #8b949e; font-size: 0.85rem; margin: 0 0 0.3rem 0; font-weight: 600;">
                    PREDICTED REVENUE
                </p>
                <p style="color: #e6edf3; font-size: 2.2rem; font-weight: 700; margin: 0;">
        ...
        st.markdown(
            f"""
            <div style="background: #1c2333; border-left: 4px solid #4CAF50;
                        padding: 1rem; border-radius: 4px; margin-top: 0.5rem;">
                <p style="margin: 0; font-size: 0.85rem; color: #e6edf3;">
                    <strong>Business Impact</strong><br>
                    Accurate demand prediction saves <strong style="color: #FFD700;">$35K-$60K annually</strong>
